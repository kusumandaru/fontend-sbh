<template>
  <b-card no-body>
    ...
    >
      ...
      <b-card-text>Point Taken from minimum {{ projectAssessment.proposed_level.score }} score</b-card-text>
    </b-col>
  ...
  setup() {
    ...
    store.dispatch('app-fa/fetchProjectAssessment', { taskId: router.currentRoute.params.id })
      .then(response => {
        ...
        // goalOverviewRadialBar.value.series = [Math.round(((projectAssessment.value.approved_score + projectAssessment.value.submitted_score) / projectAssessment.value.proposed_level.score) * 100)]
        let scoreSeries = Math.round(((projectAssessment.value.approved_score + projectAssessment.value.submitted_score) / projectAssessment.value.proposed_level.score) * 100)
