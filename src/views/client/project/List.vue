...
export default {
  ...
  methods: {
    retrieveProjects() {
      this.$http.get('engine-rest/new-building/tasks/client/pagi')
        .then(res => {
    ...
    loadItems() {
      ...
      this.$http.post('engine-rest/new-building/tasks/client/pagi', request)
        .then(res => {
          this.totalRecords = res.data.count
          this.rows = res.data.sbh_tasks
        })
